...
Require Import hpattern vgtac Fold DFMapAVL DLat DUnit DNList DStr DProd
        DMap DSum DPow DPos InterCfg UserInputType Global.
...
Module ExtProcPos <: KEY := ProdKey2 Proc DPos.DPos.
Module PowExtProcPos <: LAT := Pow ExtProcPos.
...
Definition proc_of_allocsite (x : Allocsite.t) : option Proc.t :=
  match x with
    | Allocsite.Inr (ExtAllocsite.Inr p) => Some p
    | _ => None
  end.
